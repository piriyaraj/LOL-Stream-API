...
class UploadYoutube:
    def __init__(self, match_data, video_file_name: str,thumbnail_file) -> None:
        ...
        if match_data['gamemode'] == "CLASSIC":
            self.__title = f"""{match_data['mvp']['name']} | {match_data['mvp']['champion']} VS {match_data['loser']} | RANKED | {match_data['date']}"""
        else:
            self.__title = f"{match_data['mvp']['name']} | {match_data['mvp']['champion']} | {match_data['gamemode']} | {match_data['date']}"
